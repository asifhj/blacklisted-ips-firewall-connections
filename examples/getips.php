...
<?php if ($search !== ''): ?>
  ...
  try
  {
    ...
    $job = $service->getJobs()->create('search source="Desktop_risks.csv" | head 10 | table IP_Address');
  ...
  <table class="table table-condensed table-striped">
    ...
    $anyRows = FALSE;
    $columnNames = NULL;
    $file_write = fopen("ip_from_splunk.csv", "w");
    ...
    foreach ($results as $result)
    {
	  ...
	  fputcsv($file_write, array($columnName));
        echo '</tr></thead>';
        echo "\n";
      }
      ...
      else if (is_array($result))
      {
        ...
        foreach ($columnNames as $columnName)
        {
	      ...
	      fputcsv($file_write, array(htmlspecialchars($cellValue)));
            }
          }
          echo '</td>';
        }
